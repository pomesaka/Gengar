...
- **F1.3.1** 収集イベントからセッション単位のサマリを計算する
  ...
  - 使用モデル一覧
  - 編集ファイル数、追加／削除行数（Edit / Write の差分から推定）
  - Bash 実行回数・失敗数
...
- **F1.4.1** 収集モードを選択可能にする: `full`（本文含む）／`summary`（要約と長さのみ）／`metadata`（メタデータのみ）
- **F1.4.2** 正規表現ベースの除外ルール（特定プロジェクトパス／特定プロンプト）
- **F1.4.3** シークレット検出（`.env` 系文字列や AWS/Stripe 形式トークン）で自動マスク
