...
- **F1.3.1** 収集イベントからセッション単位のサマリを計算する
  ...
  - コスト概算（モデル別単価テーブルで積算）
  - ツール呼び出し回数（種類別。Edit / Read / Bash / Grep / Glob / WebFetch / WebSearch / TodoWrite / Task(subagent) / MCP ツール等のツール名単位）
  - ツール失敗率（PostToolUse の `is_error` をカウント、種類別）
  - 使用モデル一覧とモデル別内訳
  - Bash 実行回数・失敗数、および **Bash 内で呼ばれた外部プログラム名** (第一トークン) 別の集計

> **Phase 1 の粒度制約 (R1.6)**: ツール引数は保存しない。**例外として Bash の第一トークン (プログラム名) のみ保存**する。編集ファイル名・編集行数・Bash コマンド全文・検索パターンなど引数依存の集計は Phase 2 以降で検討する。
...
- **F1.4.4** **ツール引数は原則保存しない** (R1.6)。**例外: Bash ツール** は実行された**外部プログラム名（第一トークン）**のみ保存する。Bash 以外のツール (Edit, Read, Grep, Glob 等) は引数を一切保存しない。複数プログラムが `&&` / `;` / `|` 等で連結されている場合は、各プログラム名を個別に記録する
...
- **F5.3.1** Phase 1 ではツール引数を保存しない (R1.6) ため、ヒントは引数に依存しないルールのみ
  - 「特定ツールの失敗率が高い」→ 失敗率のしきい値超過を通知
  - 「特定ツールの平均所要時間が極端に長い」→ 代替ツールの検討
  - 「トークン消費がセッション平均の N 倍」→ コスト高騰セッションを検出
  - 「ターン数が極端に多い／compact 頻度が高い」→ 計画性の見直し提案
- **F5.3.2** 「同一ファイル繰り返し Read」「Bash コマンド特定パターンのリトライ」など**ツール引数に依存する検出ルールは Phase 1 ではサポートしない** (Phase 2 以降)
- **F5.3.3** 検出ロジックはプラガブルに追加可能（ルールは Python／TS ファイル単位で定義）
